...
def aplicar_estilos(df):
    """Estilado para las tablas de la FASE 1: Sin decimales y todo centrado"""
    styler = df.style.set_properties(**{'text-align': 'center'})
    styler = styler.set_table_styles([dict(selector='th', props=[('text-align', 'center')])])
    
    # Forzar que los números salgan sin decimales, ignorando cadenas de texto
...
def aplicar_estilo_matriz(df, decimales=0):
    """Estilado para las matrices de la FASE 3: Centrado total y ocultación de ceros"""
    styler = df.style.set_properties(**{'text-align': 'center'})
    styler = styler.set_table_styles([dict(selector='th', props=[('text-align', 'center')])])
